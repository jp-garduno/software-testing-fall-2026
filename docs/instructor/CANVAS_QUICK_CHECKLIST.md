# Canvas Setup Quick Checklist

**Quick reference for setting up Canvas during async Weeks 1-3**

---

## 📋 Canvas Modules to Create

### Week 1: Git Fundamentals

**Module 1: Week 1 - Session 1: Git Basics**

- [ ] Copy instructions from CANVAS_ASYNC_INSTRUCTIONS.md (Session 1)
- [ ] Add links to theory files (01-05)
- [ ] Add link to exercise 01
- [ ] Due date: None (self-paced)
- [ ] Points: 0 (learning only)

**Module 2: Week 1 - Session 2: Git Collaboration**

- [ ] Copy instructions (Session 2)
- [ ] Add links to theory files (06-10)
- [ ] Add links to exercises 02-04
- [ ] Add note: Preview Homework 1
- [ ] Due date: None
- [ ] Points: 0

---

### Week 2: Software Testing Concepts

**Module 3: Week 2 - Session 3: Testing Fundamentals**

- [ ] Copy instructions (Session 3)
- [ ] Add links to theory files 01-02
- [ ] Add link to quiz
- [ ] Add link to case study 1
- [ ] Due date: None
- [ ] Points: 0

**Module 4: Week 2 - Session 4: Testing Pyramid & Principles**

- [ ] Copy instructions (Session 4)
- [ ] Add links to theory files 03-04
- [ ] Add link to concept maps
- [ ] Add links to case studies 2-3
- [ ] **Add reminder: Homework 1 DUE**
- [ ] Due date: End of Week 2
- [ ] Points: 0 (reminder only)

---

### Week 3: Static Testing

**Module 5: Week 3 - Session 5: Static Testing Setup**

- [ ] Copy instructions (Session 5)
- [ ] Add links to theory files 01-03
- [ ] Add links to exercises 01-02
- [ ] Add note: Homework 2 preview
- [ ] Due date: None
- [ ] Points: 0

**Module 6: Week 3 - Session 6: Linting Tools**

- [ ] Copy instructions (Session 6)
- [ ] Add link to theory file 04
- [ ] Add links to exercises 03-04
- [ ] Add link to Homework 3
- [ ] **Add reminder: Homework 2 DUE**
- [ ] Due date: End of Week 3
- [ ] Points: 0

---

## 📝 Canvas Assignments to Create

### Assignment 1: Homework 1 - Git Workflow

- **Title**: Homework 1: Git Workflow Project
- **Points**: 100
- **Due Date**: End of Week 2
- **Submission Type**: Website URL (GitHub PR link)
- **Instructions**:

  ```
  Complete the Git workflow homework assignment.

  📖 Assignment Details:
  https://github.com/software-testing-fall-2026/01-git/homework/homework-1.md

  📤 How to Submit:
  1. Complete the homework in a feature branch
  2. Create a Pull Request on GitHub
  3. Wait for automated grading (2-5 minutes)
  4. Review your grade in the PR comments
  5. Submit your PR URL here

  Example PR URL: https://github.com/software-testing-fall-2026/pull/123

  📚 Submission Guide:
  https://github.com/software-testing-fall-2026/docs/student/STUDENT_SUBMISSION_GUIDE.md

  ⏰ Time Estimate: 3-4 hours
  ```

### Assignment 2: Homework 2 - Testing Concepts

- **Title**: Homework 2: Testing Concepts Analysis
- **Points**: 110 (100 base + 10 bonus)
- **Due Date**: End of Week 3
- **Submission Type**: Website URL (GitHub PR link)
- **Instructions**:

  ```
  Analyze testing concepts and create test plans.

  📖 Assignment Details:
  https://github.com/software-testing-fall-2026/02-testing-concepts/homework/homework-2.md

  📤 How to Submit:
  1. Complete all parts of the homework
  2. Create a Pull Request on GitHub
  3. Wait for automated grading
  4. Submit your PR URL here

  💡 Includes bonus opportunities for extra credit!

  ⏰ Time Estimate: 3-4 hours
  ```

### Assignment 3: Homework 3 - Static Testing Setup

- **Title**: Homework 3: Static Testing Configuration
- **Points**: 110 (100 base + 10 bonus)
- **Due Date**: End of Week 4
- **Submission Type**: Website URL (GitHub PR link)
- **Instructions**:

  ```
  Set up complete static testing infrastructure.

  📖 Assignment Details:
  https://github.com/software-testing-fall-2026/03-static-testing/homework/homework-3.md

  📤 How to Submit:
  1. Configure pre-commit hooks and linters
  2. Fix at least 5 linting issues
  3. Write analysis report
  4. Create Pull Request
  5. Submit PR URL here

  💡 Bonus: CI/CD integration, security linting, style guide

  ⏰ Time Estimate: 3-4 hours
  ```

---

## 📢 Announcements to Post

### Before Week 1 Starts

**Title**: Welcome to Software Testing - Weeks 1-3 Async

```
Welcome to Software Testing Fall 2026! 👋

The first 3 weeks are ASYNC (self-paced, online) since I won't be available.

📅 Timeline:
- Week 1: Git Fundamentals (Sessions 1-2)
- Week 2: Software Testing Concepts (Sessions 3-4)
- Week 3: Static Testing (Sessions 5-6)
- Week 4: We meet in person!

📚 Everything you need is in Canvas modules and the GitHub repository.

🔗 Repository: https://github.com/jp-garduno/software-testing-fall-2026

📖 Start here: Module 1 - Week 1, Session 1

⏰ Expected time: ~4 hours per session, ~23 hours total

📝 Homework:
- Homework 1 (Git): Due end of Week 2
- Homework 2 (Testing): Due end of Week 3
- Homework 3 (Static): Due end of Week 4

💬 Use the Discussion Board for questions - help each other!

🎯 Goal: Master Git, understand testing fundamentals, and set up quality tools

See you in person Week 4! 🚀
```

### End of Week 1

**Title**: Week 1 Complete - Week 2 Starting

```
Great job completing Week 1! 🎉

✅ You should have completed:
- Git basics (theory 1-5, exercise 1)
- Git collaboration (theory 6-10, exercises 2-4)
- Development environment setup

⏰ Homework 1 Reminder:
Due END OF THIS WEEK (Week 2)!

📖 Assignment: 01-git/homework/homework-1.md
📤 Submit: GitHub PR link to Canvas
⏱️ Time: 3-4 hours

➡️ This Week (Week 2):
- Session 3: Testing fundamentals
- Session 4: Testing levels and principles

Keep up the great work! 💪
```

### End of Week 2

**Title**: Week 2 Complete - Homework 1 DUE TODAY

```
Week 2 complete! Halfway through async learning! 🎯

⚠️ HOMEWORK 1 DUE TODAY (End of Week 2)

Don't forget to:
1. Create your Pull Request
2. Wait for automated grading (2-5 min)
3. Submit PR link to Canvas

⏰ Homework 2 Reminder:
Due end of NEXT WEEK (Week 3)

📖 Assignment: 02-testing-concepts/homework/homework-2.md

➡️ This Week (Week 3):
- Static testing
- Pre-commit hooks
- Linting tools

Final async week - then we meet in person! 🚀
```

### End of Week 3

**Title**: Async Learning Complete - See You Week 4!

```
Congratulations! 🎉 You've completed 3 weeks of async learning!

⚠️ HOMEWORK 2 DUE TODAY (End of Week 3)

Submit your Testing Concepts homework before end of day.

✅ You've Learned:
- Git fundamentals and workflows
- Software testing concepts and principles
- Static testing with pre-commit and linting

⏰ Homework 3 (Static Testing):
- Due: End of Week 4 (next week)
- Start early if you haven't already!

👥 Week 4: We Meet In Person!
- Location: [Your Classroom]
- Day/Time: [Your Schedule]
- Topic: Black Box Testing

📚 Optional Prep for Week 4:
- Review Module 4 README: 04-black-box-testing/README.md
- Read about equivalence partitioning

See you in class! Excited to meet everyone! 🎓
```

---

## 📊 Canvas Gradebook Setup

### Categories

Create these assignment categories:

1. **Homework** (25%)

   - 9 assignments (HW 1-9)
   - 110 points each (100 base + 10 bonus)

2. **Exams** (55%)

   - Exam 1 (15%) - Modules 1-3
   - Exam 2 (20%) - Modules 4-6
   - Exam 3 (20%) - Modules 7-9

3. **Team Project** (20%)

   - 7 milestones throughout semester
   - 100 points total (M1: 10, M2: 10, M3: 15, M4: 20, M5: 15, M6: 15, M7: 15)

4. **Participation & Attendance** (10%)
   - In-person sessions (Weeks 4-16)

### Grading Scale

- A: 90-100%
- B: 80-89%
- C: 70-79%
- D: 60-69%
- F: Below 60%

---

## 🔗 External Links to Add

Add these as Canvas Pages or External Links:

### GitHub Repository

- **URL**: `https://github.com/jp-garduno/software-testing-fall-2026`
- **Description**: Main course repository

### Student Submission Guide

- **URL**: `https://github.com/jp-garduno/software-testing-fall-2026/blob/main/docs/student/STUDENT_SUBMISSION_GUIDE.md`
- **Description**: How to submit homework

### Resources

- **URL**: `https://github.com/jp-garduno/software-testing-fall-2026/blob/main/resources/README.md`
- **Description**: Books, tutorials, cheat sheets

### Course Timeline

- **URL**: `https://github.com/jp-garduno/software-testing-fall-2026/blob/main/TIMELINE.md`
- **Description**: Full 16-week schedule

---

## ✅ Pre-Launch Checklist

Before Week 1:

- [ ] Create all 6 Canvas modules (Sessions 1-6)
- [ ] Create 3 homework assignments
- [ ] Post welcome announcement
- [ ] Set up discussion board
- [ ] Test all GitHub links
- [ ] Verify automated grading workflow works
- [ ] Update repository URLs in all instructions
- [ ] Add office hours information
- [ ] Set up grade categories in gradebook

During async weeks:

- [ ] Monitor discussion board daily
- [ ] Post weekly reminder announcements
- [ ] Check for student questions
- [ ] Test a homework submission yourself
- [ ] Prepare for Week 4 in-person class

---

## 💡 Tips for Success

1. **Post Weekly Reminders**

   - Students forget due dates
   - Weekly announcements keep them on track

2. **Monitor Discussion Board**

   - Answer questions within 24 hours
   - Encourage peer-to-peer help

3. **Test Everything First**

   - Submit a test homework yourself
   - Verify automated grading works
   - Check all links

4. **Be Available via Email**

   - Even though async, be reachable
   - Set expectations for response time

5. **Prepare Week 4 Early**
   - First in-person class sets the tone
   - Review what students learned
   - Be ready for questions

---

**Good luck with your async teaching! This setup is designed to run smoothly even without your presence.** 🎓
